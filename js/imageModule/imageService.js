     ...
     xhrFields: {
        ...
        success : function(data){
            ...
            $('.imageInProgress').empty();
                    ...
                    console.log(data.image.url+"||"+data.image.name+"||"+data.image.id+"data.image.id");
            $compile($('.imageInProgress').attr('id',data.image.id).html('<img class="myImageActive" src="'+data.image.url+'" ><div style="height:20px;width:100%;background-color:rgba(0,0,0,0.8);margin-top:-1px;"><span style="font-size:6px;color:#fff;float:left; padding:2px;">'+data.image.name+'</span></div><div style="position:absolute;top:0px;left:0px;background-color:rgba(0,0,0,0.8);width:140px;height:120px;opacity:0;text-align:center;" class="imageOption"><a style="z-index:9999;color:#fff; margin-right:20px;display:inline-block;line-height:120px;" class="removeImageHover" ng-click="removeImage($event,'+"'"+data.image.id+"'"+')"><i class=" icon-trash icon-2x" ></i></a><a style="z-index:9999;color:#fff;display:inline-block;line-height: 120px;" class="selectedImageHover" ng-click="imageSelected('+"'"+data.image.url+"'"+')"><i class="icon-ok-sign icon-2x" ></i></a></div>'))($scope)
