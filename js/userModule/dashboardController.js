var dashboardController = angular.module('dashboardController', ['ngMaterial','AuthService','dashBoardService','toolBar']);

dashboardController.controller('dashboardController',function ($scope,$rootScope,$state,AuthService,$timeout,$stateParams,$mdToast,$document,getMyProjectsList,$document,$mdDialog,dashBoardFunctionCollection){
...
$('<span class="userImage"><img id="uImage" src="'+user[0].userPhoto+'"></span><span class="userName ng-binding"  role="button" tabindex="0"> '+user[0].userName+' </span>').prependTo($("#userProfileInDashboard"));
   	$scope.editPage = function(ev,id){
   		$state.go('homePage');
   		dashBoardFunctionCollection.saveProjectId(id);
   	}
   $scope.myEdit = function(){
        
		////////////////////////
		dashBoardFunctionCollection.removeProjectId();
		$state.go('homePage')
		///////////////////////
	...
	$scope.copyProject = function(ev,projectId,projectName){	
	   ...
	   console.log($scope.projectName+"|||||")
	      ...
	      controller: function($scope,$compile,dashBoardFunctionCollection){
			...
			$scope.projectInfo = {"id":"","projectname":"","cover":"","qrcode":""}
	           ...
	           dashBoardFunctionCollection.copyProject($scope.projectName,pId,$scope);
      		   ...
      		   dashBoardFunctionCollection.getProjectList().success(function(data){
      		   		...
      		   		$scope.projectInfo.id = data[0].id;
